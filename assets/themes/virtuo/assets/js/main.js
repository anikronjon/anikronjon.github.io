...
(function ($) {
  ...
  var tmPk = {
    ...
    bgActiveBlur: function name(params) {
      ...
      const $closeButton = $('.tmp-iv-close-button');

      function checkAndObserve() {
        const $videoCard = $('.tmp-intro-video-card-wrapper.position-right');
        ...
        // Проверяем, есть ли MutationObserver в браузере
        ...
        const observer = new MutationObserver(function (mutations) {
          mutations.forEach(function (mutation) {
            if (mutation.attributeName === 'class') {
              if ($videoCard.hasClass('is-expanded')) {
        ...
        observer.observe($videoCard[0], { attributes: true });
      }
    ...
    _clickDoc: function (e) {
      ...
      }).on('focus', 'input:not([type="radio"]):not([type="checkbox"]), textarea, select', function () {
    ...
    serviceWidget: function () {
      function serviceAnimation() {
        $(".services-widget").each(function () {
          var $servicesWidget = $(this);
          ...
          // Initial call per widget
          updateActiveService($servicesWidget.find(".current"));
      
          // Click selection per widget
